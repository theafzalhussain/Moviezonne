...
function buildCarousel() {
  ...
  const buildOne = (m, i, trackFrag, dotsFrag, thumbsFrag) => {
    /*  Genres now render as ONE chip in the meta row ("Action � Adventure")
     *  rather than a separate row of pills below it. Two reasons: it is what the
     ...
     *  that is all that is knowable before layout; the clamp still handles the
     *  viewport half.  */
    ...
    slide.innerHTML =
      ...
      '<div class="slide-content">' +
        ...
        '<div class="slide-meta">' +
          // The word "RATING" is gone in favour of a star glyph � same number,
          // less label. aria-label keeps it readable for assistive tech.
...
async function loadMovies(cat, isLoadMore = false) {
  ...
  try {
    if (cat === 'all') {
      ...
      // ? TOP RATED: IMDb-style highest rated, min vote threshold ???? reliable ho
      const res = await Promise.allSettled([
        tmdb('/movie/top_rated', { language: 'en-US', page: p1 }),
        tmdb('/movie/top_rated', { language: 'en-US', page: p2 }),
        tmdb('/discover/movie', { with_original_language: 'hi', sort_by: 'vote_average.desc', 'vote_count.gte': '150', page: p1, language: 'en-US' }),
...
(function initCollectionsHub() {
  ...
  async function openUniverse(slug, options) {
    ...
    try {
      ...
      if (detail) detail.innerHTML = '<div class="ch-detail-empty"><div class="ch-detail-empty-icon">??</div><strong>Could not load this universe.</strong><span>Please try again in a moment.</span></div>';
    }
  }
